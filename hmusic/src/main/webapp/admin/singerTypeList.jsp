...
<body>
	...
	<div class="panel admin-panel">
		<div class="panel-head">
			<strong class="icon-reorder"> 歌手类别管理</strong>
		</div>
		<div class="padding border-bottom">
			<ul class="search">
				<li><a class="button border-main icon-plus-square-o"
					href="${ctx}/singertype/addLoad"> 添加歌手类别</a>
		...
		<table class="table table-hover text-center" id="musicTable">
			<tr>
				<th width="5%">歌手类别ID</th>
				<th width="10%">歌手类别名称</th>
			...
			<c:forEach items="${singerTypeList}" var="singertype">
				<tr>
					<td><input type="checkbox" name="id[]" value="1" />${singertype.singertypeid}</td>
					<td>${singertype.singertypename}</td>
					<td>
						<div class="button-group">
							<a class="button border-main"
								href="${ctx}/singertype/editLoad?singertypeid=${singertype.singertypeid}"><span
								class="icon-edit"></span> 修改</a> <a class="button border-red"
								href="javascript:void(0)" onclick="return del(${singertype.singertypeid})"><span
...
function del(id){
	if(confirm("您确定要删除吗?")){		
		window.location.href = "${ctx}/singertype/delete?singertypeid="+id+"";
	}
}
